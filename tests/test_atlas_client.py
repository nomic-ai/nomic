...
def test_map_embeddings_dict_auto_algorithm():
    num_embeddings = 50
    embeddings = np.random.rand(num_embeddings, 10)
    data = [{"id": str(i)} for i in range(num_embeddings)]

    # Dictionary with only generic parameters defaults to algorithm="auto"
    dataset_identifier = f"unittest-dict-auto-{gen_temp_identifier()}"
    dataset = atlas.map_data(
        identifier=dataset_identifier,
        id_field="id",
        embeddings=embeddings,
        data=data,
        is_public=True,
        projection={"n_neighbors": 14, "n_epochs": 35, "spread": 0.8},  # Will use auto algorithm
    )
    assert dataset.id is not None
    projection = dataset.maps[0]
    _wait_for_projection_completion(projection)
    ...
    # Dictionary with explicit auto algorithm and mix of params
    dataset_identifier = f"unittest-dict-explicit-auto-{gen_temp_identifier()}"
    dataset = atlas.map_data(
        identifier=dataset_identifier,
        id_field="id",
        embeddings=embeddings,
        ...
        is_public=True,
        projection={
            "algorithm": "auto",  # Explicitly auto - backend will choose
            "n_neighbors": 15,
            "n_epochs": 35,
            "spread": 0.8,
            # These will only be used if backend chooses relevant algorithm
            "min_dist": 0.1,  # UMAP param
            "model": "nomic-project-v1",  # Nomic Project param
        },
    )
    assert dataset.id is not None
    projection = dataset.maps[0]
...
def test_map_embeddings_options_instances():
    num_embeddings = 50
    embeddings = np.random.rand(num_embeddings, 10)
    data = [{"id": str(i)} for i in range(num_embeddings)]

    # Test with NomicProjectOptions instance
    dataset_identifier = f"unittest-instance-nomic-{gen_temp_identifier()}"
    nomic_options = NomicProjectOptions(n_neighbors=13, n_epochs=33, model="nomic-project-v1", spread=0.9)
    dataset = atlas.map_data(
        identifier=dataset_identifier,
        id_field="id",
        embeddings=embeddings,
    ...
    # Test with UMAPOptions instance
    dataset_identifier = f"unittest-instance-umap-{gen_temp_identifier()}"
    umap_options = UMAPOptions(n_neighbors=17, n_epochs=29, min_dist=0.15, metric="cosine")
    dataset = atlas.map_data(
        identifier=dataset_identifier,
        id_field="id",
        embeddings=embeddings,
