...
from .utils import get_random_name, assert_valid_project_id
...
class CreateIndexResponse(BaseModel):
    ...
    job_id: str = Field(...,
                        description="The job_id to track the progress of the index build.")
    index_id: str = Field(...,
                          description="The unique identifier of the index being built.")
    project_id: str = Field(...,
                            description="The id of the project this map is being created in")
...
class AtlasClass(object):
                ...
                if project['modality'] == 'text':
                    if isinstance(datum[key], str) and len(datum[key]) == 0:
                        if replace_empty_string_values_with_string_null:
                            ...
                            msg = 'Datum {} had an empty string for key: {}'.format(
                                datum, key)
                            raise ValueError(msg)
    ...
    def get_tags(self, project_name: str, index_name=None):
        ...
        **Returns:** A dictionary mapping datum ids to tags.
        ...
        if len(project['atlas_indices']) == 0:
            raise Exception(
                f'There are no indices in the project `{project_name}`.')
    ...
    def add_embeddings(
        ...
        **Returns:** True on success.
        ...
        if project['modality'] != 'embedding':
            msg = 'Cannot add embedding to project with modality: {}'.format(
                project['modality'])
            raise ValueError(msg)
        ...
        try:
            self._validate_and_correct_user_supplied_metadata(
                data=data, project=project, replace_empty_string_values_with_string_null=replace_empty_string_values_with_string_null)
        ...
        def send_request(i):
            ...
            response = requests.post(
                ...
                headers=self.header,
                json={'project_id': project_id, 'embeddings': base64.b64encode(
                    bytesio.getvalue()).decode('utf-8'), 'data': data_shard},
            )
            return response
                    ...
                    if response.status_code != 200:
                        try:
                            ...
                            if response.status_code == 413:
                                ...
                                logger.error(
                                    f"Shard upload failed: {response}")
                                failed += shard_size
                                pbar.update(1)
                                response.close()
                    else:
    ...
    def map_embeddings(
        ...
        **Returns:** A link to your map.
        ...
        self._validate_map_data_inputs(
            ...
            for i in range(0, len(data), MAX_MEMORY_CHUNK):
                try:
                    ...
                    if number_of_datums_before_upload == 0:
                        logger.info(
                            "Deleting project due to failure in initial upload.")
    ...
    def delete_project(self, project_id: str):
        ...
        * **project_id** - The id of the project you want to delete.
        ...
        logger.info(
            f"Deleting project `{project['project_name']}` from organization `{organization_name}`")
    ...
    def download_embeddings(self, project_id, atlas_index_id, output_dir, num_workers=10):
        ...
        def download_shard(offset, check_access=False):
            response = requests.get(
                self.atlas_api_path +
                f"/v1/project/data/get/embedding/{project_id}/{atlas_index_id}/{offset}/{limit}",
                headers=self.header,
            )
            ...
            try:
                ...
                shard_name = '{}_{}_{}.pkl'.format(
                    atlas_index_id, offset, offset+limit)
        ...
        with tqdm(total=total_datums // limit) as pbar:
            with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
                futures = {executor.submit(
                    download_shard, cur_offset): cur_offset for cur_offset in range(0, total_datums, limit)}
