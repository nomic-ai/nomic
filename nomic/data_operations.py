...
class AtlasMapDuplicates:
    ...
    def __init__(self, projection: "AtlasProjection"):
        ...
        try:
            duplicate_fields = [
                field
                for field in projection._fetch_tiles().column_names
                if "_duplicate_class" in field
            ]
            cluster_fields = [
                field
                for field in projection._fetch_tiles().column_names
                if "_cluster" in field
            ]
            assert (
                len(duplicate_fields) > 0
        ...
        self._tb = self._tb.rename_columns(
            [self.id_field, self.duplicate_field, self.cluster_field]
        )
    ...
    def deletion_candidates(self) -> List[str]:
        ...
        """
        dupes = self.tb[self.id_field].filter(
            pc.equal(self.tb[self.duplicate_field], "deletion candidate")
        )
...
class AtlasMapTopics:
    ...
    def __init__(self, projection: "AtlasProjection"):
        ...
        try:
            ...
            topic_fields = [
                column
                for column in self._tb.column_names
                if column.startswith("_topic_depth_")
            ]
            ...
            self._tb = self._tb.select([self.id_field] + topic_fields).rename_columns(
                [self.id_field] + renamed_fields
            )
    ...
    def metadata(self) -> pandas.DataFrame:
        ...
        column_list = [
            (f"_topic_depth_{i}", f"topic_depth_{i}") for i in range(1, self.depth + 1)
        ]
        ...
        for _, row in topic_df[cols].iterrows():
            ...
            for topic_index in range(len(topics) - 1):
                ...
                if (
                    ...
                    not in topic_hierarchy[(topics[topic_index], topic_index + 1)]
                ):
                    topic_hierarchy[(topics[topic_index], topic_index + 1)].append(
                        topics[topic_index + 1]
                    )
    ...
    def group_by_topic(self, topic_depth: int = 1) -> List[Dict]:
        ...
        topic_datum_dict = (
            df.groupby(f"topic_depth_{topic_depth}")[datum_id_col].apply(set).to_dict()
        )
        topic_df = self.metadata
    ...
    def get_topic_density(self, time_field: str, start: datetime, end: datetime):
        ...
        merged_tb = self._tb.join(
            time_data, self.id_field, join_type="inner"
        ).combine_chunks()
...
class AtlasMapEmbeddings:
    ...
    def latent(self) -> np.array:
        ...
        for path in self.projection._tiles_in_order():
            ...
            files = path.parent.glob(
                path.with_suffix("").stem + "-*.embeddings.feather"
            )
            # Should there be more than 10, we need to sort by int values, not string values
            sortable = sorted(
                files, key=lambda x: int(x.with_suffix("").stem.split("-")[-1])
            )
            if len(sortable) == 0:
            ...
            for file in sortable:
                ...
                all_embeddings.append(
                    pc.list_flatten(tb["_embeddings"]).to_numpy().reshape(-1, dims)
                )
    ...
    def _download_latent(self):
        ...
        with tqdm(total=self.dataset.total_datums // limit) as pbar:
            while True:
                ...
                r = requests.post(
                    route, headers=self.projection.dataset.header, json=params
                )
                ...
                dest = (self.projection.tile_destination / tilename).with_suffix(
                    ".embeddings.feather"
                )
        ...
        for quad in tqdm(all_quads):
            ...
            while download_attempt < 3 and not download_success:
                ...
                if not path.exists() or overwrite:
                    download_feather(
                        root_url + filename, path, headers=self.dataset.header
                    )
                try:
                ...
                for file in tagged_files:
                    ...
                    if tag_definition_id in tag_definition_ids:
                        try:
                            ...
                            print(
                                "Permission denied: unable to delete outdated tag file. Skipping"
                            )
                            return
                        except Exception as e:
                            print(
                                f"Exception occurred when trying to delete outdated tag file: {e}. Skipping"
                            )
                            return
...
class AtlasMapData:
    ...
    def _read_prefetched_tiles_with_sidecars(self, additional_sidecars=None):
        ...
        root = feather.read_table(
            self.projection.tile_destination / Path("0/0/0.feather")
        )
        try:
            small_sidecars = set(
        ...
        for path in self.projection._tiles_in_order():
            ...
            for big_sidecar in additional_sidecars:
                fname = (
                    base64.urlsafe_b64encode(big_sidecar.encode("utf-8")).decode(
                        "utf-8"
                    )
    ...
    def _download_data(self, fields=None):
        ...
        if sidecars is None:
            ...
            for field in sidecars:
                assert (
                    field in self.dataset.dataset_fields
                ), f"Field {field} not found in dataset fields."
        encoded_sidecars = [
            base64.urlsafe_b64encode(sidecar.encode("utf-8")).decode("utf-8")
